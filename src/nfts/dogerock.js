...
module.exports = (bot) => {
    bot.command(["DogeRock", "dogerock", "DOGEROCK"], (ctx) => {
        let input = ctx.message.text;
        let inputArray = input.split(" "); 
        let message = ""; 
            
        if (inputArray.length == 1) { 
        message = "pls put a number after the dogerock to get your desired information";
        ctx.reply(message);
        } 
        else {
            inputArray.shift(); 
            ...
            let linkJson = `https://nftrarity.dog/nft/dogerocks/punk/${message}/json`;
            let link = `https://nftrarity.dog/nft/dogerocks/punk/${message}`;
          
            let answer1 = 
`<a href="${link}">DogeRock #${message}</a>`;
